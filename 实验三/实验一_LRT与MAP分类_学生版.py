...
class MyGaussianNB:
    ...
    def predict(self, X):
        ...
        """
        # TODO: 1. 计算对数似然
        log_likelihoods = self._calculate_log_likelihood(X)  # 形状: (n_samples, n_classes)
        
        # TODO: 2. 计算对数先验
        log_priors = np.log(self.priors_)  # 形状: (n_classes,)
        
        # TODO: 3. 计算对数后验 = 对数似然 + 对数先验
        log_joint = log_likelihoods + log_priors  # 广播机制会自动扩展log_priors
        
        # TODO: 4. 选择后验概率最大的类别
        return np.argmax(log_joint, axis=1)  # 沿类别维度取最大值索引
...
def apply_LRT_rule(model, X):
    ...
    """
    # TODO: 1. 计算对数似然
    log_likelihoods = model._calculate_log_likelihood(X)  # 形状: (n_samples, 2)
    
    # TODO: 2. 提取两个类别的对数似然
    log_like_c0 = log_likelihoods[:, 0]  # 类别0的对数似然
    log_like_c1 = log_likelihoods[:, 1]  # 类别1的对数似然
    ...
    log_threshold = log_priors[0] - log_priors[1]  # log(P(C0)) - log(P(C1))
